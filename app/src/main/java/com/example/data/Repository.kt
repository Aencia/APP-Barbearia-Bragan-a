...
class Repository(private val db: AppDatabase) {
    ...
    suspend fun ensureDefaultDataPopulated() {
        ...
        // "Atendimentos finalizados: 232, Faturamento atual: R$ 10.750,16... Taxa de retorno atual: 84%, Ticket médio atual: R$ 71,19"
        // Wait, if we average out R$ 71.19 per appointment and have 151 appointments, we get exactly R$ 10,750.16. Let's create about 35-40 random customers to reflect an 84% return rate perfectly!
        // We will generate 40 clients. To have 84% return rate, 34 of them should have more than 1 appointment (totalServices > 1).
        if (clientDao.getAllClients().isEmpty()) {
            ...
            for ((index, name) in clientsNames.withIndex()) {
                ...
                val clientObj = Client(
                    name = name,
                    phone = phone,
                    firstServiceDate = apptTime - 15 * 24 * 60 * 60 * 1000L, // 15 days earlier first contact
                    lastServiceDate = apptTime,
                    totalServices = 0 // Will increment as we seed appointments
                )
                val cId = clientDao.insertClient(clientObj).toInt()
                addedClients.add(clientObj.copy(id = cId))
            }

            // Seed 151 appointments to exactly total R$ 10,750.16 (approx R$ 71.19 per ticket)
            ...
            for (i in 0 until totalAppointmentsNeeded) {
                ...
                // Pick a client: use clients dynamically to build retention. Let's sample clients.
                // To get 84% return rate, we will pick clients with a weighted distribution.
                val clientIndex = if (i < 30) {
                    i % addedClients.size
                } else {
                    // Pick from first 30 clients to ensure they return multiple times!
                    random.nextInt(30 % addedClients.size)
                }
                val client = addedClients[clientIndex]

                // Construct services / products
                // Target around R$ 71.19
                // Let's combine standard service and product
                ...
                // Randomly add a product for some bills to match ~71.19 average
                ...
                // Fine-tune last item to touch 10,750.16 exactly
                ...
                // Choose a random timestamp spread across this current month
                ...
                // Save appointment and update details
